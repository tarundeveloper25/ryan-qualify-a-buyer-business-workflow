#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────────────
# gabriel-operator workflow-builder skills — curl installer
#
# Usage:
...
OWNER="go-code-bot"
REPO="go-workflow-builder-skills"
BRANCH="main"
...
install_skills() {
  ...
  # Copy package.json alongside so __dirname resolution works
  if command -v curl &>/dev/null; then
...
header "🔧 gabriel-operator workflow-builder skills installer"
check_node
install_skills
