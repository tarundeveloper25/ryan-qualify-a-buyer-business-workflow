...
/**
 * go-workflow-builder-skills CLI
 *
 * Usage:
 *   npx github:go-code-bot/go-workflow-builder-skills             # install into ./
 *   npx github:go-code-bot/go-workflow-builder-skills add ./path  # install into target dir
 *   npx github:go-code-bot/go-workflow-builder-skills sync ./path # re-sync (overwrite)
 *
...
const REPO = 'go-workflow-builder-skills';
const BRANCH = 'main';
...
function fetch(url, opts = {}) {
  return new Promise((resolve, reject) => {
    const options = {
      headers: {
        'User-Agent': 'go-workflow-builder-skills-cli/1.0',
...
function fetchBuffer(url) {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { 'User-Agent': 'go-workflow-builder-skills-cli/1.0' } }, (res) => {
      if (res.statusCode === 301 || res.statusCode === 302) {
...
async function installSkills(targetDir, { overwrite = false } = {}) {
  ...
  console.log(`\n\x1b[1m🔧 gabriel-operator workflow-builder skills\x1b[0m`);
  ...
  // Filter out CLI infrastructure files
  ...
  if (written > 0) {
    ...
    console.log(`  1. Review SKILL.md for usage instructions.`);
    console.log(`  2. Run validation:  npx tsx scripts/validate-workflow.ts assets/workflow.json`);
    console.log(`  3. Generate example: npx tsx scripts/generate-example.ts\n`);
  }
}
...
const COMMANDS = { add: false, sync: true, install: false };

async function main() {
  // Default: no command → install into cwd
  if (!command || command === 'add' || command === 'install') {
    ...
    console.log(`
\x1b[1mgabriel-operator workflow-builder skills CLI\x1b[0m

Usage:
  npx github:go-code-bot/go-workflow-builder-skills [command] [target-dir]
