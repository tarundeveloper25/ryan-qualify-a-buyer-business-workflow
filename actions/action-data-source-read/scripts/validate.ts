#!/usr/bin/env npx tsx
/**
 * Validates a single data_source_read step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-data-source-read/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "data_source_read") err(`${p}.action_type`, 'must be "data_source_read"');
  ...
  // dataSourceReadConfig is required (connectorId, kind, operation are required within it)
  if (!isObject(step.dataSourceReadConfig)) {
    err(`${p}.dataSourceReadConfig`, "must be an object");
  } else {
    const cfg = step.dataSourceReadConfig as Record<string, unknown>;
    if (typeof cfg.connectorId !== "string" || cfg.connectorId.trim() === "") {
      err(`${p}.dataSourceReadConfig.connectorId`, "must be a non-empty string");
    }
    if (typeof cfg.kind !== "string" || cfg.kind.trim() === "") {
      err(`${p}.dataSourceReadConfig.kind`, "must be a non-empty string");
    }
    if (typeof cfg.operation !== "string" || cfg.operation.trim() === "") {
      err(`${p}.dataSourceReadConfig.operation`, "must be a non-empty string");
    }
  }
}
