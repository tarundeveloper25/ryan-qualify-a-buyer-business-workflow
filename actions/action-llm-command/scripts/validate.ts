#!/usr/bin/env npx tsx
/**
 * Validates a single llm_command step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-llm-command/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "llm_command") err(`${p}.action_type`, 'must be "llm_command"');
