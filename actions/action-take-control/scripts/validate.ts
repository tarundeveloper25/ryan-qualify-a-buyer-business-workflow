#!/usr/bin/env npx tsx
/**
 * Validates a single take_control step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-take-control/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "take_control") err(`${p}.action_type`, 'must be "take_control"');
