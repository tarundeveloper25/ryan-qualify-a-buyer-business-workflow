#!/usr/bin/env npx tsx
/**
 * Validates a single fill step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-fill/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "fill") err(`${p}.action_type`, 'must be "fill"');
  ...
  if (step.constraints !== undefined && !isObject(step.constraints)) {
    err(`${p}.constraints`, "must be an object");
  }
