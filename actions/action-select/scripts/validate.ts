#!/usr/bin/env npx tsx
/**
 * Validates a single select step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-select/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "select") err(`${p}.action_type`, 'must be "select"');
