#!/usr/bin/env npx tsx
/**
 * Validates a single manual_extract step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-manual-extract/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "manual_extract") err(`${p}.action_type`, 'must be "manual_extract"');
