#!/usr/bin/env npx tsx
/**
 * Validates a single switch_tab step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-switch-tab/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "switch_tab") err(`${p}.action_type`, 'must be "switch_tab"');
  ...
  if (step.data !== undefined) {
    if (!isObject(step.data)) {
      ...
      if (data.tabIndex !== undefined && typeof data.tabIndex !== "number") {
        err(`${p}.data.tabIndex`, "must be a number (zero-based tab index)");
      }
    }
  }
}
