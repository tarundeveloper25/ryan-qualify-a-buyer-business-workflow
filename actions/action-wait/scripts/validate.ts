#!/usr/bin/env npx tsx
/**
 * Validates a single wait step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-wait/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "wait") err(`${p}.action_type`, 'must be "wait"');
  ...
  // Optional value: if present must be a string parseable as a non-negative float (seconds).
  if (step.value !== undefined) {
    if (typeof step.value !== "string") {
      err(`${p}.value`, 'must be a string representing seconds (e.g. "2" or "0.5")');
    } else {
      const parsed = parseFloat(step.value as string);
      if (isNaN(parsed) || parsed < 0) {
        err(`${p}.value`, 'must be a string representing a non-negative number of seconds (e.g. "2" or "0.5")');
      }
    }
  }
}
