#!/usr/bin/env npx tsx
/**
 * Validates a single scroll step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-scroll/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "scroll") err(`${p}.action_type`, 'must be "scroll"');
  ...
  if (step.data !== undefined && !isObject(step.data)) {
    err(`${p}.data`, "must be an object with x and y pixel offsets");
  }
