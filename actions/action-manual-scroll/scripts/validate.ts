#!/usr/bin/env npx tsx
/**
 * Validates a single manual_scroll step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-manual-scroll/scripts/validate.ts <step.json>
 */
...
const SCROLL_DIRECTIONS = ["down", "up", "left", "right"] as const;
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "manual_scroll") err(`${p}.action_type`, 'must be "manual_scroll"');
  ...
  if (step.scrollDirection !== undefined) {
    if (!SCROLL_DIRECTIONS.includes(step.scrollDirection as never)) {
      err(`${p}.scrollDirection`, `must be one of: ${SCROLL_DIRECTIONS.join(", ")}`);
    }
  }
