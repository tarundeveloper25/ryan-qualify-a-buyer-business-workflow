#!/usr/bin/env npx tsx
/**
 * Validates a single pdf_response step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-pdf-response/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "pdf_response") err(`${p}.action_type`, 'must be "pdf_response"');
