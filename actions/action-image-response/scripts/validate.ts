#!/usr/bin/env npx tsx
/**
 * Validates a single image_response step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-image-response/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "image_response") err(`${p}.action_type`, 'must be "image_response"');
  ...
  if (step.fullPage !== undefined && typeof step.fullPage !== "boolean") {
    err(`${p}.fullPage`, "must be a boolean");
  }
  if (step.maxImages !== undefined) {
    if (typeof step.maxImages !== "number") {
      err(`${p}.maxImages`, "must be a number");
    } else if ((step.maxImages as number) > 100) {
      err(`${p}.maxImages`, "must not exceed 100");
    }
  }
}
