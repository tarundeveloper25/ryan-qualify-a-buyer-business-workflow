#!/usr/bin/env npx tsx
/**
 * Validates a single keypress step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-keypress/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "keypress") err(`${p}.action_type`, 'must be "keypress"');
  ...
  if (step.value !== undefined && typeof step.value !== "string") {
    err(`${p}.value`, "must be a string (key name, e.g. 'Enter', 'Tab', 'Escape')");
  }
}
