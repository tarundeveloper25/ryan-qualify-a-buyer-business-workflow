#!/usr/bin/env npx tsx
/**
 * Validates a single upload step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-upload/scripts/validate.ts <step.json>
 */
...
const UPLOAD_TYPES = ["session", "file"] as const;
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "upload") err(`${p}.action_type`, 'must be "upload"');
  ...
  if (step.value !== undefined && typeof step.value !== "string") {
    err(`${p}.value`, "must be a string (filename to upload)");
  }
  if (step.uploadType !== undefined) {
    if (!UPLOAD_TYPES.includes(step.uploadType as never)) {
      err(`${p}.uploadType`, `must be one of: ${UPLOAD_TYPES.join(", ")}`);
    }
  }
}
