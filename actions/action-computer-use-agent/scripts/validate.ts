#!/usr/bin/env npx tsx
/**
 * Validates a single computer_use_agent step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-computer-use-agent/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "computer_use_agent") err(`${p}.action_type`, 'must be "computer_use_agent"');
  ...
  if (step.sandboxAgentConfig !== undefined) {
    if (!isObject(step.sandboxAgentConfig)) {
      err(`${p}.sandboxAgentConfig`, "must be an object");
    } else {
      const cfg = step.sandboxAgentConfig as Record<string, unknown>;
      if (cfg.agentType !== undefined && cfg.agentType !== "computer_use") {
        err(`${p}.sandboxAgentConfig.agentType`, 'must be "computer_use"');
      }
      if (cfg.sandboxProvider !== undefined && typeof cfg.sandboxProvider !== "string") {
        err(`${p}.sandboxAgentConfig.sandboxProvider`, "must be a string");
      }
      if (cfg.sandboxTimeout !== undefined && typeof cfg.sandboxTimeout !== "number") {
        err(`${p}.sandboxAgentConfig.sandboxTimeout`, "must be a number (milliseconds)");
      }
      if (cfg.visionModel !== undefined && typeof cfg.visionModel !== "string") {
        err(`${p}.sandboxAgentConfig.visionModel`, "must be a string");
      }
    }
  }
}
