#!/usr/bin/env npx tsx
/**
 * Validates a single api_output step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-api-output/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "api_output") err(`${p}.action_type`, 'must be "api_output"');
  ...
  if (step.outputName !== undefined && typeof step.outputName !== "string") {
    err(`${p}.outputName`, "must be a string");
  }
  if (step.outputFields !== undefined) {
    if (!Array.isArray(step.outputFields)) {
      err(`${p}.outputFields`, "must be an array");
    } else {
      for (let i = 0; i < (step.outputFields as unknown[]).length; i++) {
        const field = (step.outputFields as unknown[])[i];
        if (!isObject(field)) {
          err(`${p}.outputFields[${i}]`, "must be an object with key and value");
        } else {
          const f = field as Record<string, unknown>;
          if (typeof f.key !== "string") err(`${p}.outputFields[${i}].key`, "must be a string");
          if (typeof f.value !== "string") err(`${p}.outputFields[${i}].value`, "must be a string");
        }
      }
    }
  }
  if (step.outputSchema !== undefined && typeof step.outputSchema !== "string") {
    err(`${p}.outputSchema`, "must be a JSON schema string");
  }
}
