#!/usr/bin/env npx tsx
/**
 * Validates a single download step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-download/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "download") err(`${p}.action_type`, 'must be "download"');
