#!/usr/bin/env npx tsx
/**
 * Validates a single continuous_screenshots step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-continuous-screenshots/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "continuous_screenshots") err(`${p}.action_type`, 'must be "continuous_screenshots"');
  ...
  if (step.enablePrompt !== undefined && typeof step.enablePrompt !== "boolean") {
    err(`${p}.enablePrompt`, "must be a boolean");
  }
  if (step.data !== undefined) {
    if (!isObject(step.data)) {
      err(`${p}.data`, "must be an object");
    } else {
      const data = step.data as Record<string, unknown>;
      if (data.interval !== undefined && typeof data.interval !== "number") {
        err(`${p}.data.interval`, "must be a number (milliseconds between screenshots)");
      }
      if (data.duration !== undefined && typeof data.duration !== "number") {
        err(`${p}.data.duration`, "must be a number (total capture duration in milliseconds)");
      }
    }
  }
