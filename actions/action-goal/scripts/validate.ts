#!/usr/bin/env npx tsx
/**
 * Validates a single goal step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-goal/scripts/validate.ts <step.json>
 */
...
const NOTIFICATION_CHANNELS = ["email", "slack", "sendgrid", "webhook", "whatsapp", "whatsapp-web", "telegram"] as const;
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "goal") err(`${p}.action_type`, 'must be "goal"');
  ...
  if (typeof step.userPrompt !== "string" || step.userPrompt.trim() === "") {
    err(`${p}.userPrompt`, "must be a non-empty string");
  }

  // Optional fields with constraints
  if (step.goalMaxSteps !== undefined && typeof step.goalMaxSteps !== "number") {
    err(`${p}.goalMaxSteps`, "must be a number");
  }
  if (step.goalNotificationChannel !== undefined) {
    if (!NOTIFICATION_CHANNELS.includes(step.goalNotificationChannel as never)) {
      err(`${p}.goalNotificationChannel`, `must be one of: ${NOTIFICATION_CHANNELS.join(", ")}`);
    }
  }
}
