#!/usr/bin/env npx tsx
/**
 * Validates a single blank_step step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-blank-step/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "blank_step") err(`${p}.action_type`, 'must be "blank_step"');
