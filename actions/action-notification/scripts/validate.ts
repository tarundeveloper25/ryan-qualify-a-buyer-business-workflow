#!/usr/bin/env npx tsx
/**
 * Validates a single notification step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-notification/scripts/validate.ts <step.json>
 */
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "notification") err(`${p}.action_type`, 'must be "notification"');
