#!/usr/bin/env npx tsx
/**
 * Validates a single coding_agent step object.
 * Usage: npx tsx server/skills/workflow-builder/actions/action-coding-agent/scripts/validate.ts <step.json>
 */
...
const CODING_MODELS = ["claude_code", "codex", "amp", "open_code", "custom"] as const;
...
function validateStep(step: unknown): void {
  ...
  if (step.action_type !== "coding_agent") err(`${p}.action_type`, 'must be "coding_agent"');
  ...
  if (step.sandboxAgentConfig !== undefined) {
    if (!isObject(step.sandboxAgentConfig)) {
      ...
      if (cfg.agentType !== undefined && cfg.agentType !== "coding") {
        err(`${p}.sandboxAgentConfig.agentType`, 'must be "coding"');
      }
      if (cfg.codingModel !== undefined && !CODING_MODELS.includes(cfg.codingModel as never)) {
        err(`${p}.sandboxAgentConfig.codingModel`, `must be one of: ${CODING_MODELS.join(", ")}`);
      }
